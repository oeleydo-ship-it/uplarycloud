...
class BackupController extends Controller
{
    ...
    {
        $limits->enforceFeature($context->current(), 'backups');
        $limits->enforce($context->current(), 'backups');
        $limits->enforce($context->current(), 'backup_storage_gb', 0);
        $data = $request->validate(['application_deployment_id' => ['required', 'exists:application_deployments,id'], 'backup_destination_id' => ['nullable', 'exists:backup_destinations,id'], 'backup_type' => ['required', 'in:application,database,volume,full'], 'name' => ['nullable', 'string', 'max:150']]);
        ...
        $destination = ! empty($data['backup_destination_id']) ? BackupDestination::where('tenant_id', $context->id())->findOrFail($data['backup_destination_id']) : null;
        $backup = Backup::create(['tenant_id' => $context->id(), 'application_deployment_id' => $deployment->id, 'server_id' => $deployment->server_id, 'backup_destination_id' => $destination?->id, 'created_by' => $request->user()->id, 'name' => ($data['name'] ?? null) ?: $deployment->name.' '.ucfirst($data['backup_type']).' '.now()->format('Y-m-d H:i'), 'backup_type' => $data['backup_type'], 'status' => 'pending']);
        CreateBackupJob::dispatch($backup->id, $context->id(), $request->user()->id);

        return back()->with('success', 'Backup queued.');
    }

    public function schedule(Request $request, TenantContext $context, PlanLimitService $limits): RedirectResponse
    {
        $limits->enforceFeature($context->current(), 'backups');
        $data = $request->validate(['application_deployment_id' => ['required', 'exists:application_deployments,id'], 'backup_destination_id' => ['nullable', 'exists:backup_destinations,id'], 'name' => ['required', 'string', 'max:120'], 'backup_type' => ['required', 'in:application,database,volume,full'], 'frequency' => ['required', 'in:hourly,daily,weekly'], 'keep_last' => ['required', 'integer', 'between:1,100'], 'delete_after_days' => ['required', 'integer', 'between:1,3650']]);
        $deployment = ApplicationDeployment::where('tenant_id', $context->id())->with('server')->findOrFail($data['application_deployment_id']);
        $this->authorize('operate', $deployment->server);
        if (! empty($data['backup_destination_id'])) {
            BackupDestination::where('tenant_id', $context->id())->findOrFail($data['backup_destination_id']);
        }BackupSchedule::create($data + ['tenant_id' => $context->id(), 'enabled' => true, 'next_run_at' => now()]);
    ...
    public function destination(Request $request, TenantContext $context, PlanLimitService $limits): RedirectResponse
    {
        $server = Server::where('tenant_id', $context->id())->firstOrFail();
        $this->authorize('operate', $server);
        $data = $request->validate(['name' => ['required', 'string', 'max:100'], 'provider' => ['required', Rule::in(['local', 's3', 'r2', 'b2', 'spaces', 'custom_s3'])], 'endpoint' => ['nullable', 'url', 'max:500', Rule::requiredIf(fn () => in_array($request->input('provider'), ['r2', 'b2', 'spaces', 'custom_s3'], true))], 'bucket' => ['nullable', 'string', 'max:120', Rule::requiredIf(fn () => $request->input('provider') !== 'local')], 'region' => ['nullable', 'string', 'max:60'], 'access_key' => ['nullable', 'string', 'max:500', Rule::requiredIf(fn () => $request->input('provider') !== 'local')], 'secret_key' => ['nullable', 'string', 'max:1000', Rule::requiredIf(fn () => $request->input('provider') !== 'local')]]);
        if (($data['provider'] ?? 'local') !== 'local') {
            $limits->enforceFeature($context->current(), 's3_destinations');
        }BackupDestination::create($data + ['tenant_id' => $context->id(), 'active' => true, 'last_verified_at' => $data['provider'] === 'local' ? now() : null]);

        return back()->with('success', 'Encrypted backup destination saved. It will be verified by the first backup.');
    }

    public function restore(Request $request, Backup $backup, TenantContext $context): RedirectResponse
    {
        $this->operate($backup, $context);
        RestoreBackupJob::dispatch($backup->id, $context->id(), $request->user()->id);

        return back()->with('success', 'Restore queued.');
    }

    public function download(Backup $backup, TenantContext $context, BackupService $service): BinaryFileResponse
    {
        $this->guard($backup, $context);
        $this->authorize('view', $backup->server);
        try {
            $path = $service->downloadPath($backup);
        } catch (\RuntimeException) {
            abort(404);
        }

return response()->download($path, $backup->uuid.'.tar.gz', ['Content-Type' => 'application/gzip']);
    }

    public function destroy(Backup $backup, TenantContext $context, BackupService $service): RedirectResponse
    {
        $this->operate($backup, $context);
    ...
    private function operate(Backup $backup, TenantContext $context): void
    {
        $this->guard($backup, $context);
        $this->authorize('operate', $backup->server);
    }

    private function guard(Backup $backup,TenantContext $context): void
    {
        abort_unless($backup->tenant_id === $context->id(),404);
    }
}
