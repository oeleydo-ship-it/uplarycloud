...
class ApiTokenController extends Controller
{
    ...
    public function index(Request $request, TenantContext $context): View
    {
        ...
        if ($request->status === 'active') $query->whereNull('revoked_at')->where(fn ($q) => $q->whereNull('expires_at')->orWhere('expires_at', '>', now()));
        if ($request->status === 'expired') $query->whereNull('revoked_at')->where('expires_at', '<=', now());
        if ($request->status === 'revoked') $query->whereNotNull('revoked_at');
        if ($request->filled('permission')) $query->whereJsonContains('abilities', $request->permission);
    ...
    {
        $this->access($request); $data = $this->validated($request);
    ...
    {
        $this->access($request); $this->guard($request, $token, $context); $data = $this->validated($request);
    ...
    {
        $this->access($request); $this->guard($request, $token, $context); $token->update(['revoked_at'=>now()]);
    ...
    private function validated(Request $request): array { return $request->validate(['name'=>['required','string','max:100'],'scopes'=>['required','array','min:1'],'scopes.*'=>[Rule::in(self::SCOPES)],'environment'=>['required','in:production,staging,development'],'expires_in'=>['required','in:30,90,365,never'],'ip_restrictions'=>['nullable','string','max:1000']]); }
    private function expiry(string $value) { return $value === 'never' ? null : now()->addDays((int) $value); }
    private function ips(string $value): array { return collect(preg_split('/[\s,]+/', $value))->filter()->each(fn ($ip) => abort_unless(filter_var($ip,FILTER_VALIDATE_IP),422))->values()->all(); }
    private function guard(Request $request, PersonalAccessToken $token, TenantContext $context): void { abort_unless($token->tenant_id === $context->id() && $token->tokenable_id === $request->user()->id, 404); }
    private function access(Request $request): void { $role=$request->user()->tenants()->whereKey(session('tenant_id'))->first()?->pivot->role; abort_unless(in_array($role,['owner','admin','developer'],true),403); }
    private function log(Request $request, TenantContext $context, string $action, string $description, array $scopes=[]): void { ActivityLog::create(['tenant_id'=>$context->id(),'user_id'=>$request->user()->id,'action'=>$action,'description'=>$description,'ip_address'=>$request->ip(),'status'=>'success','metadata'=>['scopes'=>$scopes]]); }
}
