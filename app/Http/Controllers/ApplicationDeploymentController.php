...
class ApplicationDeploymentController extends Controller
{
    ...
    public function install(Application $application, TenantContext $context): View { abort_unless($application->active,404); return $this->wizard($application,$context); }
    public function custom(TenantContext $context): View { return $this->wizard(null,$context); }
    public function store(StoreApplicationDeploymentRequest $request, TenantContext $context): RedirectResponse
    {
        $data=$request->validated(); $server=Server::where('tenant_id',$context->id())->findOrFail($data['server_id']); $this->authorize('operate',$server);
        if ($server->status !== ServerStatus::Online) throw ValidationException::withMessages(['server_id'=>'Select an online server.']);
        if (($data['memory_limit_mb']??0)>$server->memory_mb || ($data['disk_limit_gb']??0)>$server->disk_gb) throw ValidationException::withMessages(['server_id'=>'The selected server does not meet the requested resources.']);
        $deployment=DB::transaction(function () use ($data,$request,$server,$context): ApplicationDeployment {
            ...
            foreach ($data['environment_keys']??[] as $index=>$key) if ($key) $deployment->environmentVariables()->create(['key'=>$key,'value'=>$data['environment_values'][$index]??'','secret'=>in_array((string)$index,$data['environment_secrets']??[],true),'description'=>$data['environment_descriptions'][$index]??null]);
            return $deployment;
        });
    ...
    public function show(ApplicationDeployment $deployment, TenantContext $context): View { $this->guard($deployment,$context); $this->authorize('view',$deployment->server); return view('applications.show',['deployment'=>$deployment->load(['application.category','buildPack','server','steps','logs','environmentVariables','releases'])]); }
    public function status(ApplicationDeployment $deployment, TenantContext $context): JsonResponse { $this->guard($deployment,$context); return response()->json(['status'=>$deployment->status->value,'progress'=>$deployment->progress,'stage'=>$deployment->current_stage,'error'=>$deployment->last_error,'steps'=>$deployment->steps()->get(['key','name','status','error']),'logs'=>$deployment->logs()->latest('occurred_at')->limit(100)->get()->reverse()->values()->map(fn($log)=>['level'=>$log->level,'message'=>$log->message,'time'=>$log->occurred_at->format('H:i:s')])]); }
    ...
    public function rollback(Request $request, ApplicationDeployment $deployment, DeploymentRelease $release, TenantContext $context): RedirectResponse { $this->guard($deployment,$context); $this->authorize('operate',$deployment->server); abort_unless($release->application_deployment_id===$deployment->id,404); if ($release->is_current) return back()->withErrors(['rollback'=>'That release is already active.']); RollbackApplicationDeploymentJob::dispatch($deployment->id,$release->id,$context->id(),$request->user()->id); event(new DeploymentProgressed($deployment->tenant_id,$deployment->uuid,'rolling_back',$deployment->progress,'rollback')); return back()->with('success','Rollback queued.'); }
    ...
    private function guard(ApplicationDeployment $deployment,TenantContext $context): void { abort_unless($deployment->tenant_id===$context->id(),404); }
}
