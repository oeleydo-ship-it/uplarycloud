...
{
    public function accrue(Server $server, ?InfrastructureOperation $operation = null): InfrastructureCharge
    {
        $plan = $server->managedPlan;
        $start = now()->startOfMonth();
        $prepaid = (bool) ($operation?->parameters['prepaid'] ?? false);

        return InfrastructureCharge::firstOrCreate(
            ['tenant_id' => $server->tenant_id, 'server_id' => $server->id, 'charge_type' => 'managed_server', 'period_starts_at' => $start],
            [
                'infrastructure_operation_id' => $operation?->id,
                'description' => $server->name.' · '.$plan->name,
                'quantity' => 1,
                'unit_amount' => $plan->monthly_price,
                'total' => $plan->monthly_price,
                'currency' => $plan->currency,
                'status' => $prepaid ? 'paid' : 'pending',
                'period_ends_at' => $start->copy()->endOfMonth(),
                'metadata' => [
                    'provider' => $server->provider->value,
                    'plan' => $plan->provider_plan_id,
                    'provider_cost' => $plan->monthly_cost,
                    'prepaid' => $prepaid,
                ],
            ]
        );
